...
public class AuthController {
     ...
     */
    public LoginResult recuperarSenha(String email) {
        if (!networkClient.isConnected()) {
            return new LoginResult(false, "Não conectado ao servidor", null);
        }
        
        String comando = CMD_RECOVER_PASSWORD + SEPARATOR + email;
        String resposta = networkClient.sendCommand(comando);
        ...
        if (partes.length >= 1) {
            ...
            switch (status) {
                case STATUS_OK:
                    String mensagem = partes.length >= 2 ? partes[1] : "Nova senha enviada por email";
                    return new LoginResult(true, mensagem, null);
                    
                case STATUS_INVALID_DATA:
                    String erro = partes.length >= 2 ? partes[1] : "Dados inválidos";
                    return new LoginResult(false, erro, null);
                    
                default:
                    String erroMsg = partes.length >= 2 ? partes[1] : "Erro desconhecido";
                    return new LoginResult(false, erroMsg, null);
            }
        }
