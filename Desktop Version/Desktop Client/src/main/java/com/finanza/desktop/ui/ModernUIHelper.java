...
public class ModernUIHelper {
    ...
    /**
     * Cria um botão de navegação com ícone real do projeto
     */
    public static JButton createNavButton(String text, String iconUnicode) {
        JButton button = new JButton();
        
        // Tentar carregar ícone real baseado no texto
        ImageIcon realIcon = getIconForNavigation(text);
        if (realIcon != null) {
            button.setIcon(realIcon);
            button.setText(text);
            button.setHorizontalTextPosition(SwingConstants.RIGHT);
            button.setIconTextGap(8);
        } else {
            // Fallback para unicode
            button.setText(iconUnicode + " " + text);
        }
        ...
        addHoverEffect(button, PRIMARY_DARK_BLUE);
        
        return button;
    }
    
    /**
     * Mapeia texto de navegação para ícones reais
     */
    private static ImageIcon getIconForNavigation(String text) {
        switch (text.toLowerCase()) {
            case "dashboard":
                return IconLoader.loadIcon(IconLoader.HOME);
            case "contas":
                return IconLoader.loadIcon(IconLoader.BANK);
            case "movimentações":
                return IconLoader.loadIcon(IconLoader.ARROWS);
            case "perfil":
                return IconLoader.loadIcon(IconLoader.MENU); // usando menu como profile
            case "configurações":
                return IconLoader.loadIcon(IconLoader.MENU);
            case "sair":
                return IconLoader.loadIcon(IconLoader.CLOSE);
            default:
                return null;
        }
    }
